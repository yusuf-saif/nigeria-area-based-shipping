...
class NGABS_Importer {
	...
	public static function import_csv($tmp_path) {
		...
		if (!$handle) {
			return new WP_Error('ngabs_csv_open_failed', __('Failed to open CSV file.', 'ngabs'));
		}
		...
		if (!$header) {
			fclose($handle);
			return new WP_Error('ngabs_csv_empty', __('CSV is empty.', 'ngabs'));
		}
		...
		while (($row = fgetcsv($handle)) !== false) {
			...
			if ($state_raw === '' && $area_raw === '' && $price_raw === '') {
				continue;
			}
			...
			$existing = false;
			$areas = NGABS_DB::list_areas($state_code);
			foreach ($areas as $a) {
				if ((string) $a['area_name'] === (string) $area_raw) {
					$existing = true;
					break;
				}
			}
